...
namespace REH_Presentacion.ActivoFijo.Reportes
{
    ...
    /// </summary>
    public partial class frmListadoMovimientoActivoFijo : frmBaseTrxDev
    {
        ...
        #region Eventos
        public frmListadoMovimientoActivoFijo()
        {
            InitializeComponent();
        ...
        private void frmTrNomina_Load(object sender, EventArgs e)
        {
            try
            {
                clsComun.gLLenarCombo(ref cmbTipoMovimiento, loLogicaNegocio.goConsultarComboTipoMovimientoActivo(),false, false, "", true);
        ...
        {
            return loLogicaNegocio.goConsultaDataTable(string.Format("EXEC AFISPLISTADOMOVIMIENTOSACTIVOS {0}", cmbTipoMovimiento.EditValue));
        }
        ...
        private void lBuscar()
        {
            ...
            if (int.Parse(Tag.ToString().Split(',')[0]) == 246 || int.Parse(Tag.ToString().Split(',')[0]) == 247 || int.Parse(Tag.ToString().Split(',')[0]) == 343)
            {
