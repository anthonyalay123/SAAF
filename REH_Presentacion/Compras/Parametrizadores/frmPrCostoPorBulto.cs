...
namespace REH_Presentacion.Compras.Parametrizadores
{
    ...
    {
        #region Variables
        clsNCostoPorBulto loLogicaNegocio = new clsNCostoPorBulto();
        ...
        private List<Combo> loComboBodega = new List<Combo>();
        #endregion
        ...
        private void frmPrExcepcionClienteComision_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                loComboBodega = loLogicaNegocio.goSapConsultaComboCatalogoBodegas();
                ...
                clsComun.gLLenarComboGrid(ref dgvDatos, loComboBodega, "WhsCode", true);
                clsComun.gLLenarComboGrid(ref dgvDatos, loLogicaNegocio.goConsultarComboTipoTransporte(), "CodigoTipoTransporte", true);
                lBuscar();
            }
            catch (Exception ex)
            {
        ...
        private void btnGrabar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    List<CostoPorBulto> poLista = (List<CostoPorBulto>)bsDatos.DataSource;
        ...
        private void rpiBtnDel_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            try
            {
                ...
                var poLista = (List<CostoPorBulto>)bsDatos.DataSource;
        ...
        private void btnAddManualmente_Click(object sender, EventArgs e)
        {
            ...
            {
                bsDatos.AddNew();
                ((List<CostoPorBulto>)bsDatos.DataSource).LastOrDefault().CodigoTipoTransporte = Diccionario.Seleccione;
                ((List<CostoPorBulto>)bsDatos.DataSource).LastOrDefault().CardCode = Diccionario.Seleccione;
                ((List<CostoPorBulto>)bsDatos.DataSource).LastOrDefault().CodeZona = Diccionario.Seleccione;
                ((List<CostoPorBulto>)bsDatos.DataSource).LastOrDefault().WhsCode = Diccionario.Seleccione;
                dgvDatos.Focus();
        ...
        private string lsEsValido()
        {
            ...
            var poListaRecorrida = new List<CostoPorBulto>();
            int fila = 1;
            foreach (var item in poLista)
            {
                ...
                string psBodega = loComboBodega.Where(x => x.Codigo.Contains(item.WhsCode)).Select(x => x.Descripcion).FirstOrDefault();

                if (item.CodeZona == Diccionario.Seleccione || item.CardCode == Diccionario.Seleccione || item.WhsCode == Diccionario.Seleccione || item.CodigoTipoTransporte == Diccionario.Seleccione)
                {
                    psMsg = string.Format("{0}Verificar si están seleccionados el Tipo de Transporte, Almacén, Proveedor y la Zona en la Fila # {1}\n", psMsg, fila);
                }

                else if (poListaRecorrida.Where(x => x.CodeZona == item.CodeZona && x.CardCode == item.CardCode).Count() > 0)
                {
                    psMsg = string.Format("{0}Eliminar Fila # {1}, Ya existe una parametrización por: {2}, {3}, {4} y {5}\n", psMsg, fila, psCliente, psZona, psBodega,item.CodigoTipoTransporte);
                }
        ...
        private void lCargarEventosBotones()
        {
            ...
            bsDatos.DataSource = new List<CostoPorBulto>();
            ...
            dgvDatos.Columns["IdCostoPorBulto"].Visible = false;
            dgvDatos.Columns["WhsName"].Visible = false;

            dgvDatos.Columns["CardCode"].Caption = "Proveedor";
            dgvDatos.Columns["CodeZona"].Caption = "Zona";
            dgvDatos.Columns["WhsCode"].Caption = "Almacen";
            dgvDatos.Columns["CodigoTipoTransporte"].Caption = "Tipo Transporte";
