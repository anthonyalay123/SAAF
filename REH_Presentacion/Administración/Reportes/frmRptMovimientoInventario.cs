...
namespace REH_Presentacion.Administracion.Reportes
{
    ...
    /// </summary>
    public partial class frmRptMovimientoInventario : frmBaseTrxDev
    {
        ...
        #region Eventos
        public frmRptMovimientoInventario()
        {
            InitializeComponent();
        ...
        private void frmTrNomina_Load(object sender, EventArgs e)
        {
            try
            {
                clsComun.gLLenarCombo(ref cmbCentroCosto, loLogicaNegocio.goConsultarComboCentroCosto(), false, false, "TODOS");
                clsComun.gLLenarCombo(ref cmbTipo, loLogicaNegocio.goConsultarComboTipoItem(), false, false, "TODOS");
        ...
        private void btnGenerarDiario_Click(object sender, EventArgs e)
        {
            try
            {
                XtraMessageBox.Show("No existen confirugadas los centros de costos con sus cuentas contables.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            catch (Exception ex)
            {
                XtraMessageBox.Show(ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void btnImprimir_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                var ds = loLogicaNegocio.goConsultaDataSet(string.Format("EXEC ADMSPMOVIMIENTOINVENTARIO '{0}','{1}','{2}','{3}'", dtpFechaInicial.DateTime, dtpFechaFinal.DateTime, cmbTipo.EditValue.ToString(), cmbCentroCosto.EditValue.ToString()));

                ds.Tables[0].TableName = "MovimientoInventario";

                if (ds.Tables[0].Rows.Count > 0)
                {
                    xrptListadoMovimientoInventario xrpt = new xrptListadoMovimientoInventario();
                    xrpt.DataSource = ds;
                    xrpt.Parameters["parameter1"].Value = string.Format("LISTADO DE MOVIMIENTOS DEL {0} AL {1}.\n TIPO DE ÍTEMS: {2} \n CENTRO DE COSTO: {3}", dtpFechaInicial.DateTime.ToString("dd/MM/yyyy"), dtpFechaFinal.DateTime.ToString("dd/MM/yyyy"), cmbTipo.Text, cmbCentroCosto.Text);
        ...
        {
            string psQuery = string.Format("EXEC ADMSPMOVIMIENTOINVENTARIO '{0}','{1}','{2}','{3}'", dtpFechaInicial.DateTime, dtpFechaFinal.DateTime, cmbTipo.EditValue.ToString(), cmbCentroCosto.EditValue.ToString());
        ...
        private void lBuscar()
        {
           ...
           // dgvDatos.OptionsSelection.ShowCheckBoxSelectorInGroupRow = DevExpress.Utils.DefaultBoolean.True;
            ...
            dgvDatos.OptionsCustomization.AllowSort = false;

            dgvDatos.OptionsView.ShowGroupedColumns = true;
            dgvDatos.Columns["Bodega"].GroupIndex = 0;
            dgvDatos.Columns["Item"].GroupIndex = 1;
            dgvDatos.ExpandAllGroups();

            dgvDatos.Columns["Bodega"].Visible = false;
            dgvDatos.Columns["Item"].Visible = false;

            dgvDatos.Appearance.GroupFooter.Font = new Font("Tahoma", 8.25F, System.Drawing.FontStyle.Bold);
            this.dgvDatos.OptionsView.ShowFooter = false;
