...
namespace REH_Presentacion.Ventas.Transacciones
{
    ...
    public partial class frmRpEnviarRebateMail : frmBaseTrxDev
    {
        ...
        #region Eventos
        public frmRpEnviarRebateMail()
        {
            InitializeComponent();
            cmbAsunto.EditValueChanged += cmbAsunto_EditValueChanged;
        }
        ...
        private void frmTrNomina_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                clsComun.gLLenarCombo(ref cmbAsunto, loLogicaNegocio.goConsultarComboAsuntoRebate(), true);
        ...
        private void cmbAsunto_EditValueChanged(object sender, EventArgs e)
        {
            lConsultarBoton();
        }
        ...
        private void btnEnviarCorreo_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (lbEsValido())
                {
                    string codigoAsunto = cmbAsunto.EditValue.ToString();

                    int anioAsunto = int.Parse(codigoAsunto.Substring(0, 4));
                    int trimestreAsunto = int.Parse(codigoAsunto.Substring(4, 1));

                    DataTable dt = loLogicaNegocio.goConsultaDataTable(string.Format("EXEC VTASPREBATEPENDPAGOXENVIARMAIL '{0}', '{1}', '1', '{2}', '{3}'", anioAsunto, trimestreAsunto, frmCorreo.TraerHtmlDelRichEdit(), txtFirma.Text));
                    if (dt != null && dt.Rows.Count > 0)
                    {
                        List<Attachment> listAdjuntosEmail = new List<Attachment>();

                        string psRuta = ConfigurationManager.AppSettings["FileTmpCom"].ToString() + clsComun.gRemoverCaracteresEspeciales(cmbAsunto.Text.Trim()) + ".xlsx";
                        gcDatos.ExportToXlsx(psRuta);
                        
                        if (File.Exists(psRuta))
                            listAdjuntosEmail.Add(new Attachment(psRuta));
                        
                        var msg = loLogicaNegocio.EnviarPorCorreo(txtDestinatarios.Text, cmbAsunto.Text, dt.Rows[0][0].ToString(), listAdjuntosEmail, true);
                        //var msg = loLogicaNegocio.EnviarPorCorreo("pasante_sistemas@afecor.com", cmbAsunto.Text, dt.Rows[0][0].ToString(), listAdjuntosEmail, true);
        ...
        {
            if (string.IsNullOrEmpty(cmbAsunto.Text))
            {
        ...
        private void lBuscar()
        {
            ...
            // Assuming the selected value format is "YYYYQ" (e.g., 20231 for Q1 2023)
            int anioAsunto = int.Parse(codigoAsunto.Substring(0, 4));
            int trimestreAsunto = int.Parse(codigoAsunto.Substring(4, 1));

            //DataTable dt = loLogicaNegocio.goConsultaDataTable(string.Format("EXEC COMSPFACTURASPENDPAGOXENVIARMAIL"));
            DataTable dt = loLogicaNegocio.goConsultaDataTable(string.Format("EXEC VTASPREBATEPENDPAGOXENVIARMAIL '{0}', '{1}'", anioAsunto, trimestreAsunto));
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                //txtAsunto.Text = string.Format("REBATE Q{0}/{1} {2} GRUPO", psMes, psAnio, psMes == "1" ? "PRIMER" : psMes == "2" ? "SEGUNDO" : psMes == "3" ? "TERCER" : "CUARTO");

                string psTexto = loLogicaNegocio.goConsultarParametroVta().TextoEmailRebateTrimestral;

                string psVal1 = string.Format("Q{0}/{1} {2} GRUPO", psMes, psAnio, psMes == "1" ? "PRIMER" : psMes == "2" ? "SEGUNDO" : psMes == "3" ? "TERCER" : "CUARTO");
                ...
                //txtTexto.Text = psTexto.Replace("#Val1", psVal1).Replace("#Val2", psVal2);
        ...
        private void lConsultarBoton()
        {
            try
            {
                ...
                if(cmbAsunto.EditValue.ToString() != "0")
                {
                    lBuscar();
                }
