...
{
    public partial class frmTrBandejaAprobacionRebateAnual : frmBaseTrxDev
    {
        ...
        public frmTrBandejaAprobacionRebateAnual()
        {
            InitializeComponent();
        ...
        private void frmTrBandejaAprobacionRebate_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                clsComun.gLLenarCombo(ref cmbPeriodo, loLogicaNegocio.gsConsultarComboParaRebate("VTASPCONSULTABANDEJAREBATEANUAL", clsPrincipal.gsUsuario, Int32.Parse(menu[0]), "Periodo", "Periodo"), false);
            ...
            try
            {
                ...
                if (poLista.Count() > 0)
                {
                    ...
                    if (dialogResult == DialogResult.Yes)
                    {
                        ...
                        {
                            var psMess = loLogicaNegocio.gsAprobarRebateAnual(item.Id, item.Observacion, item.PorcentajeRebate, item.ValorRebate, clsPrincipal.gsUsuario, clsPrincipal.gsTerminal);
            ...
            try
            {
                ...
                if (poLista.Count() > 0)
                {
                    ...
                    if (dialogResult == DialogResult.Yes)
                    {
                        ...
                        foreach (var item in poLista)
                        {
                            var psMess = loLogicaNegocio.gsAprobacionDefinitivaRebateAnual(item.Id, item.Observacion, item.PorcentajeRebate, item.ValorRebate, clsPrincipal.gsUsuario, clsPrincipal.gsTerminal);
                            if (!string.IsNullOrEmpty(psMess))
                            {
                                msg = string.Format("{0}{1}\n", msg, psMess);
                            }
            ...
            try
            {
                ...
                if (poLista.Count() > 0)
                {
                    ...
                    if (dialogResult == DialogResult.Yes)
                    {
                        ...
                        foreach (var item in poLista)
                        {
                            var psMess = loLogicaNegocio.gsDesaprobarRebateAnual(item.Id, item.Observacion, clsPrincipal.gsUsuario, clsPrincipal.gsTerminal);
                            if (!string.IsNullOrEmpty(psMess))
                            {
                                msg = string.Format("{0}{1}\n", msg, psMess);
                            }
                ...
                if (poLista.Count > 0)
                {
                    if (string.IsNullOrEmpty(psMsg))
                    {
                        ...
                        if (dialogResult == DialogResult.Yes)
                        {
                            ...
                            foreach (var item in poLista)
                            {
                                loLogicaNegocio.gActualizarEstadoRebateAnual(item.Id, Diccionario.Corregir, item.Observacion, clsPrincipal.gsUsuario, clsPrincipal.gsTerminal);
                            }
                            XtraMessageBox.Show(Diccionario.MsgRegistroGuardado, Diccionario.MsgTituloRegistroGuardado, MessageBoxButtons.OK, MessageBoxIcon.Information);
                            lListar();
                        }
                ...
                if (poLista.Count > 0)
                {
                    if (string.IsNullOrEmpty(psMsg))
                    {
                        ...
                        if (dialogResult == DialogResult.Yes)
                        {
                            ...
                            foreach (var item in poLista)
                            {
                                loLogicaNegocio.gActualizarEstadoRebateAnual(item.Id, Diccionario.Rechazado, item.Observacion, clsPrincipal.gsUsuario, clsPrincipal.gsTerminal);
                            }
                            XtraMessageBox.Show(Diccionario.MsgRegistroGuardado, Diccionario.MsgTituloRegistroGuardado, MessageBoxButtons.OK, MessageBoxIcon.Information);
                            lListar();
                        }
        ...
        private void lListar()
        {
            bsDatos.DataSource = new List<BandejaAprobacionRebate>();
            var menu = Tag.ToString().Split(',');
            var poObject = loLogicaNegocio.goListarBandejaRebateAnual(clsPrincipal.gsUsuario, Int32.Parse(menu[0]), cmbPeriodo.EditValue.ToString());
            //if (poObject != null)
            //{
