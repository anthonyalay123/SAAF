...
{
    public partial class frmPaPresupuestoKilosLitrosGrupo : frmBaseTrxDev
    {
        ...
        List<Combo> loComboGrupo;
        #endregion

        public frmPaPresupuestoKilosLitrosGrupo()
        {
            InitializeComponent();
        ...
        private void frmPaPresupuestoVentasRebate_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                clsComun.gLLenarCombo(ref cmbGrupo, loComboGrupo, true);
        ...
        private void btnPlantilla_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (lbCargado)
                {
                    ...
                    bs.DataSource = new List<PresupuestoKilosLitrosGrupoExcel>();
                    dgv.BestFitColumns();
            ...
            try
            {
                ...
                if (ofdRuta.ShowDialog() == DialogResult.OK)
                {
                    if (!ofdRuta.FileName.Equals(""))
                    {
                        ...
                        var poLista = (List<PresupuestoKilosLitrosGrupoGrid>)bsDatos.DataSource;
                        var poListaImportada = new List<PresupuestoKilosLitrosGrupoGrid>();
                        ...
                        foreach (DataRow item in dt.Rows)
                        {
                            ...
                            {
                                PresupuestoKilosLitrosGrupoGrid poItem = new PresupuestoKilosLitrosGrupoGrid();
                                poItem.Periodo = int.Parse(item[0].ToString().Trim());
                                poItem.CodeZona = item[1].ToString().Trim();
                                poItem.NameZona = item[2].ToString().Trim();
                                poItem.GroupCode = short.Parse(item[3].ToString().Trim());
                                poItem.GroupName = item[4].ToString().Trim();
                                ...
                                if (loComboZona.Where(x=>x.Codigo == poItem.CodeZona).Count() > 0 && loComboGrupo.Where(x => short.Parse(x.Codigo)  == poItem.GroupCode).Count() > 0)
                                {
                                ...
                                else
                                {
                                    ...
                                    if (loComboGrupo.Where(x => short.Parse(x.Codigo)  == poItem.GroupCode).Count() == 0)
                                    {
                                        psMsgImport = string.Format("{0}Fila #{1} Columna 'Codigo Grupo' contiene un valor no encontradoen Base de Datos. Valor: '{2}'.\n", psMsgImport, piContador, poItem.CodeZona);
                                    }
                                }
        ...
        private void btnGrabar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                List<PresupuestoKilosLitrosGrupoGrid> poLista = (List<PresupuestoKilosLitrosGrupoGrid>)bsDatos.DataSource;

                if (poLista.Count > 0)
                {
                    string psMsg = loLogicaNegocio.gsGuardarGrupo(poLista, clsPrincipal.gsUsuario, clsPrincipal.gsTerminal);
                    if (string.IsNullOrEmpty(psMsg))
                    {
        ...
        private void lCargarEventosBotones()
        {
            ...
            bsDatos.DataSource = new List<PresupuestoKilosLitrosGrupoGrid>();
            gcParametrizaciones.DataSource = bsDatos;
            dgvParametrizaciones.Columns["IdPresupuestoKilosLitrosGrupo"].Visible = false;
            ...
            dgvParametrizaciones.Columns["GroupCode"].OptionsColumn.AllowEdit = false;
            dgvParametrizaciones.Columns["GroupName"].OptionsColumn.AllowEdit = false;
            ...
            cmbGrupo.KeyDown += new KeyEventHandler(EnterEqualTab);
        ...
        private string lsEsValido(bool tbGenerar = true)
        {
            ...
            if (cmbGrupo.EditValue.ToString() == Diccionario.Seleccione)
            {
                psMsg = string.Format("{0}Seleccione Grupo. \n", psMsg);
            }
        ...
        {
            if ((cmbGrupo.Properties.DataSource as IList).Count > 0) cmbGrupo.ItemIndex = 0;
        ...
        private void lBuscar()
        {
            ...
            if (!string.IsNullOrEmpty(txtAnio.Text))
            {
                ...
                var poLIsta = loLogicaNegocio.goConsultarParametrizacionesGrupo(piPeriodo);
                bsDatos.DataSource = poLIsta;
                gcParametrizaciones.DataSource = poLIsta;
            }
            else
            {
                var poLIsta = new List<PresupuestoKilosLitrosGrupoGrid>();
                bsDatos.DataSource = poLIsta;
                gcParametrizaciones.DataSource = poLIsta;
            }
        ...
        private void btnAddFila_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(psMsgValida))
                {
                    ...
                    var poLista = (List<PresupuestoKilosLitrosGrupoGrid>)bsDatos.DataSource;
                    var poListaAgregar = new List<PresupuestoKilosLitrosGrupoGrid>();
                    PresupuestoKilosLitrosGrupoGrid poItem = new PresupuestoKilosLitrosGrupoGrid();
                    ...
                    poItem.GroupCode = short.Parse(cmbGrupo.EditValue.ToString());
                    poItem.GroupName = cmbGrupo.Text;
        ...
        private string lsValidaDuplicados(List<PresupuestoKilosLitrosGrupoGrid> toListaGrid, List<PresupuestoKilosLitrosGrupoGrid> toListaInsertar)
        {
            string psMsg = string.Empty;

            foreach (var item in toListaInsertar)
            {
                var piRegistro = toListaGrid.Where(x => x.Periodo == item.Periodo && x.GroupCode == item.GroupCode && x.CodeZona == item.CodeZona).Count();
                if(piRegistro > 0)
                {
                    psMsg = string.Format("{0}Periodo: {1} con la zona: {2} y Grupo: {3} Ya están parametrizados. \n", psMsg, item.Periodo, item.NameZona, item.GroupName);
                }
            }
