...
namespace REH_Presentacion.Ventas.Parametrizadores
{
    ...
    /// </summary>
    public partial class frmPrZonasEnvioCorreo : frmBaseTrxDev
    {
        #region Variables
        ...
        private void rpiBtnDet_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            try
            {
                ...
                if (poLista.Count > 0 && piIndex >= 0)
                {
                    // Tomamos la entidad de la fila seleccionada
                    // Tomamos la entidad de la fila seleccionada
                    var poDetalle = poLista[piIndex].ZonaGrupoEnvioCorreo == null ? new List<ZonaGrupoEnvioCorreo>() : poLista[piIndex].ZonaGrupoEnvioCorreo.ToList();

                    frmZonaGrupoCorreoPersona frm = new frmZonaGrupoCorreoPersona();
                    frm.Detalle = poDetalle;
                    frm.ShowDialog();

                    if (frm.pbAcepto)
                    {
                        poLista[piIndex].ZonaGrupoEnvioCorreo = frm.Detalle;
                    }
                }
            }
            catch (Exception ex)
