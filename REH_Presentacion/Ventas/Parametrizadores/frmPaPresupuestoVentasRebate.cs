...
{
    public partial class frmPaPresupuestoVentasRebate : frmBaseTrxDev
    {
        ...
        List<Combo> loListaClientes;
        List<Combo> loListaZonas;

        #endregion

        public frmPaPresupuestoVentasRebate()
        {
            InitializeComponent();
        ...
        private void frmPaPresupuestoVentasRebate_Load(object sender, EventArgs e)
        {
            ...
            {
                lbCargado = false;
                loListaClientes = loLogicaNegocio.goSapConsultaClientes();
                clsComun.gLLenarCombo(ref cmbCliente, loListaClientes, true);
        ...
        private void btnPlantilla_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (true)
                {
                    ...
                    DataTable dt = loLogicaNegocio.gdtPlantillaPResupuestoRebate(piPeriodo);
                    if (dt.Rows.Count > 0)
                    {
            ...
            try
            {
                ...
                if (ofdRuta.ShowDialog() == DialogResult.OK)
                {
                    if (!ofdRuta.FileName.Equals(""))
                    {
                        ...
                        var poLista = (List<PresupuestoRebatePivotGrid>)bsDatos.DataSource;
                        var poListaImportada = new List<PresupuestoRebatePivotGrid>();
                        ...
                        foreach (DataRow item in dt.Rows)
                        {
                            ...
                            {
                                PresupuestoRebatePivotGrid poItem = new PresupuestoRebatePivotGrid();
                                poItem.Periodo = int.Parse(item[0].ToString().Trim());
                                //poItem.CodeVendedor = int.Parse(item[1].ToString().Trim());
                                //poItem.NameVendedor = item[2].ToString().Trim();
                                //poItem.CodeZona = item[3].ToString().Trim();
                                ...
                                //poItem.CodeCliente = item[5].ToString().Trim();
                                poItem.NameCliente = item[2].ToString().Trim();
                                string psCodeGC = loListaClientes.Where(x => x.Descripcion == poItem.NameCliente).Select(x => x.Codigo).FirstOrDefault();
                                if (!string.IsNullOrEmpty(psCodeGC))
                                {
                                    poItem.CodeCliente = psCodeGC;
                                }
                                else
                                {
                                    psMsgLista = string.Format("{0}No se encontró Cliente con el nombre: '{1}'  en la fila {2}.\n", psMsgLista, poItem.NameCliente, fila);
                                }
        ...
        private void btnGrabar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                List<PresupuestoRebatePivotGrid> poLista = (List<PresupuestoRebatePivotGrid>)bsDatos.DataSource;
        ...
        private void lCargarEventosBotones()
        {
            ...
            bsDatos.DataSource = new List<PresupuestoRebatePivotGrid>();
            gcParametrizaciones.DataSource = bsDatos;
            dgvParametrizaciones.Columns["IdPresupuestoRebate"].Visible = false;
            dgvParametrizaciones.Columns["CodeZona"].Visible = false;
            dgvParametrizaciones.Columns["CodeCliente"].Visible = false;
            dgvParametrizaciones.OptionsBehavior.Editable = true;
            dgvParametrizaciones.Columns["Periodo"].OptionsColumn.AllowEdit = false;
            //dgvParametrizaciones.Columns["CodeVendedor"].OptionsColumn.AllowEdit = false;
            //dgvParametrizaciones.Columns["NameVendedor"].OptionsColumn.AllowEdit = false;
            ...
            dgvParametrizaciones.Columns["CodeCliente"].OptionsColumn.AllowEdit = false;
            dgvParametrizaciones.Columns["NameCliente"].OptionsColumn.AllowEdit = false;
        ...
        private void lBuscar()
        {
            if (!string.IsNullOrEmpty(txtAnio.Text))
            {
                ...
                var poLIsta = loLogicaNegocio.goConsultarParametrizaciones(piPeriodo);
                bsDatos.DataSource = poLIsta;
            ...
            {
                var poLIsta = new List<PresupuestoRebatePivotGrid>();
                bsDatos.DataSource = poLIsta;
                gcParametrizaciones.DataSource = poLIsta;
            }
        ...
        private void btnAddFila_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(psMsgValida))
                {
                    ...
                    var poLista = (List<PresupuestoRebatePivotGrid>)bsDatos.DataSource;
                    var poListaAgregar = new List<PresupuestoRebatePivotGrid>();
                    PresupuestoRebatePivotGrid poItem = new PresupuestoRebatePivotGrid();
                    poItem.Periodo = int.Parse(txtAnio.Text.Trim());
                    //poItem.CodeVendedor = int.Parse(cmbVendedor.EditValue.ToString());
                    //poItem.NameVendedor = cmbVendedor.Text;
                    poItem.CodeZona = cmbZona.EditValue.ToString();
                    poItem.NameZona = cmbZona.Text;
                    poItem.CodeCliente = cmbCliente.EditValue.ToString();
                    poItem.NameCliente = cmbCliente.Text;
        ...
        private string lsValidaDuplicados(List<PresupuestoRebatePivotGrid> toListaGrid, List<PresupuestoRebatePivotGrid> toListaInsertar)
        {
            string psMsg = string.Empty;

            foreach (var item in toListaInsertar)
            {
                var piRegistro = toListaGrid.Where(x => x.Periodo == item.Periodo && x.CodeCliente == item.CodeCliente && x.CodeZona == item.CodeZona).Count();
                if(piRegistro > 0)
                {
                    psMsg = string.Format("{0}Zona: {1} y Cliente: {2} Ya están parametrizados. \n", psMsg, item.NameZona, item.NameCliente);
                }
            }
        ...
        private void lImprimir()
        {
            ...
            var poLista = (List<PresupuestoRebatePivotGrid>)bsDatos.DataSource;
            if (poLista[piIndex].IdPresupuestoRebate.ToString() != "")
            {

                var poCartaRebate = loLogicaNegocio.goCartaRebate(poLista[piIndex].CodeCliente, int.Parse(txtAnio.Text)-1);
                ...
                xrpt.Parameters["Cliente"].Value = poLista[piIndex].NameCliente;
