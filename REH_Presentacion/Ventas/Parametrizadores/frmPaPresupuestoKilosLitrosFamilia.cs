...
{
    public partial class frmPaPresupuestoKilosLitrosFamilia : frmBaseTrxDev
    {
        ...
        List<Combo> loComboFamilia;
        #endregion

        public frmPaPresupuestoKilosLitrosFamilia()
        {
            InitializeComponent();
        ...
        private void frmPaPresupuestoVentasRebate_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                loComboFamilia = loLogicaNegocio.goConsultarComboSapFamilia();
                clsComun.gLLenarCombo(ref cmbFamilia, loComboFamilia, true);
        ...
        private void btnPlantilla_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (lbCargado)
                {
                    ...
                    bs.DataSource = new List<PresupuestoKilosLitrosFamiliaExcel>();
                    dgv.BestFitColumns();
            ...
            try
            {
                ...
                if (ofdRuta.ShowDialog() == DialogResult.OK)
                {
                    if (!ofdRuta.FileName.Equals(""))
                    {
                        ...
                        var poLista = (List<PresupuestoKilosLitrosFamiliaGrid>)bsDatos.DataSource;
                        var poListaImportada = new List<PresupuestoKilosLitrosFamiliaGrid>();
                        ...
                        foreach (DataRow item in dt.Rows)
                        {
                            ...
                            {
                                PresupuestoKilosLitrosFamiliaGrid poItem = new PresupuestoKilosLitrosFamiliaGrid();
                                poItem.Periodo = int.Parse(item[0].ToString().Trim());
                                poItem.CodeZona = item[1].ToString().Trim();
                                poItem.NameZona = item[2].ToString().Trim();
                                poItem.Familia = item[3].ToString().Trim();
                                poItem.Ene = Convert.ToDecimal(item[4].ToString().Trim());
                                poItem.Feb = Convert.ToDecimal(item[5].ToString().Trim());
                                poItem.Mar = Convert.ToDecimal(item[6].ToString().Trim());
                                poItem.Abr = Convert.ToDecimal(item[7].ToString().Trim());
                                poItem.May = Convert.ToDecimal(item[8].ToString().Trim());
                                poItem.Jun = Convert.ToDecimal(item[9].ToString().Trim());
                                poItem.Jul = Convert.ToDecimal(item[10].ToString().Trim());
                                poItem.Ago = Convert.ToDecimal(item[11].ToString().Trim());
                                poItem.Sep = Convert.ToDecimal(item[12].ToString().Trim());
                                poItem.Oct = Convert.ToDecimal(item[13].ToString().Trim());
                                poItem.Nov = Convert.ToDecimal(item[14].ToString().Trim());
                                poItem.Dic = Convert.ToDecimal(item[15].ToString().Trim());
                                poItem.Observacion = item[16].ToString().Trim();
                                piContador++;

                                if (loComboZona.Where(x=>x.Codigo == poItem.CodeZona).Count() > 0 && loComboFamilia.Where(x => x.Descripcion  == poItem.Familia).Count() > 0)
                                {
                                ...
                                else
                                {
                                    ...
                                    if (loComboFamilia.Where(x => x.Descripcion  == poItem.Familia).Count() == 0)
                                    {
                                        psMsgImport = string.Format("{0}Fila #{1} Columna 'Familia' contiene un valor no encontradoen Base de Datos. Valor: '{2}'.\n", psMsgImport, piContador, poItem.CodeZona);
                                    }
                                }
        ...
        private void btnGrabar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                List<PresupuestoKilosLitrosFamiliaGrid> poLista = (List<PresupuestoKilosLitrosFamiliaGrid>)bsDatos.DataSource;

                if (poLista.Count > 0)
                {
                    string psMsg = loLogicaNegocio.gsGuardarFamilia(poLista, clsPrincipal.gsUsuario, clsPrincipal.gsTerminal);
                    if (string.IsNullOrEmpty(psMsg))
                    {
        ...
        private void lCargarEventosBotones()
        {
            ...
            bsDatos.DataSource = new List<PresupuestoKilosLitrosFamiliaGrid>();
            gcParametrizaciones.DataSource = bsDatos;
            dgvParametrizaciones.OptionsBehavior.Editable = true;

            dgvParametrizaciones.Columns["IdPresupuestoKilosLitrosFamilia"].Visible = false;
            ...
            cmbFamilia.KeyDown += new KeyEventHandler(EnterEqualTab);
        ...
        private string lsEsValido(bool tbGenerar = true)
        {
            ...
            if (cmbFamilia.EditValue.ToString() == Diccionario.Seleccione)
            {
                psMsg = string.Format("{0}Seleccione Familia. \n", psMsg);
            }
        ...
        {
            if ((cmbFamilia.Properties.DataSource as IList).Count > 0) cmbFamilia.ItemIndex = 0;
        ...
        private void lBuscar()
        {
            ...
            if (!string.IsNullOrEmpty(txtAnio.Text))
            {
                ...
                var poLIsta = loLogicaNegocio.goConsultarParametrizacionesFamilia(piPeriodo);
                bsDatos.DataSource = poLIsta;
                gcParametrizaciones.DataSource = poLIsta;
            }
            else
            {
                var poLIsta = new List<PresupuestoKilosLitrosFamiliaGrid>();
                bsDatos.DataSource = poLIsta;
                gcParametrizaciones.DataSource = poLIsta;
            }
        ...
        private void btnAddFila_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(psMsgValida))
                {
                    ...
                    var poLista = (List<PresupuestoKilosLitrosFamiliaGrid>)bsDatos.DataSource;
                    var poListaAgregar = new List<PresupuestoKilosLitrosFamiliaGrid>();
                    PresupuestoKilosLitrosFamiliaGrid poItem = new PresupuestoKilosLitrosFamiliaGrid();
                    ...
                    poItem.Familia = cmbFamilia.Text;
        ...
        private string lsValidaDuplicados(List<PresupuestoKilosLitrosFamiliaGrid> toListaGrid, List<PresupuestoKilosLitrosFamiliaGrid> toListaInsertar)
        {
            string psMsg = string.Empty;

            foreach (var item in toListaInsertar)
            {
                var piRegistro = toListaGrid.Where(x => x.Periodo == item.Periodo && x.Familia == item.Familia && x.CodeZona == item.CodeZona).Count();
                if(piRegistro > 0)
                {
                    psMsg = string.Format("{0}Periodo: {1} con la zona: {2} y Familia: {3} Ya están parametrizados. \n", psMsg, item.Periodo, item.NameZona, item.Familia);
                }
            }
