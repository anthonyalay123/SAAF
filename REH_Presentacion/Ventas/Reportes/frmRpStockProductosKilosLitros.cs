...
namespace REH_Presentacion.Ventas.Reportes
{
    ...
    /// </summary>
    public partial class frmRpStockProductosKilosLitros : frmBaseTrxDev
    {
        ...
        #region Eventos
        public frmRpStockProductosKilosLitros()
        {
            InitializeComponent();
        ...
        private void frmTrNomina_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                Lista.Add(new Combo() { Codigo = "0", Descripcion = "Uni y Kls Ltrs"});
                Lista.Add(new Combo() { Codigo = "1", Descripcion = "Solo Unidades" });
                Lista.Add(new Combo() { Codigo = "2", Descripcion = "Solo Kilos Litros" });
                clsComun.gLLenarCombo(ref cmbUnidades, Lista);
                cmbBodega.Properties.DataSource = loLogicaNegocio.goSapConsultaComboBodegas();
                cmbBodega.Properties.ValueMember = "Codigo";
                cmbBodega.Properties.DisplayMember = "Descripcion";
                cmbBodega.Properties.PopupSizeable = true;
        ...
        {
            if (!string.IsNullOrEmpty(cmbBodega.EditValue.ToString()))
            {
                string psBodegas = cmbBodega.EditValue.ToString().Replace(" ", "");
                string psSoloKL = cmbUnidades.EditValue.ToString();
                return loLogicaNegocio.goConsultaDataTable(string.Format("EXEC VTASPSTOCKPRODUCTOSKILOSLITROS '{0}','{1}'", psBodegas, psSoloKL));
            }
            else
            {
                XtraMessageBox.Show("Seleccione Bodegas", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void lBuscar()
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                dgvDatos.Columns[4].Fixed = FixedStyle.Left;
