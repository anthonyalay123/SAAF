...
namespace REH_Presentacion.Ventas.Reportes
{
    ...
    /// Fecha: 08/04/2020
    /// Formulario de reporte de Cumplimiento de Kilos Litros GV
    /// </summary>
    public partial class frmRpCumplimientoKilosLitrosGV : frmBaseTrxDev
    {
        ...
        #region Eventos
        public frmRpCumplimientoKilosLitrosGV()
        {
            InitializeComponent();
        ...
        private void btnImprimir_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (dt1.Rows.Count > 0)
                {
                    ...
                    psParametro1 = string.Format("VENTAS DEL {0} AL {1} (GV)", dtpFechaInicial.Value.ToString("dd/MM/yyyy"), dtpFechaFinal.Value.ToString("dd/MM/yyyy"));
                    ...
                    ////Se establece el origen de datos del reporte (El dataset previamente leído)
                    //xrpt2.Parameters["Titulo"].Value = string.Format("VENTAS DEL {0} AL {1}. ZONA: {2}. (GV)", dtpFechaInicial.Value.ToString("dd/MM/yyyy"), dtpFechaFinal.Value.ToString("dd/MM/yyyy"), cmbZonas.Text);
        ...
        //{
        //    return loLogicaNegocio.goConsultaDataTable(string.Format("EXEC [dbo].[VTA_SP_CUMPLIMIENTO_KILOS_LITROS_X_ZONA_GV] '{0}', '{1}', {2} ", dtpFechaInicial.Value, dtpFechaFinal.Value, cmbZonas.EditValue.ToString()));
        //}

        private DataTable ldtValidaQuery()
        {
            string psZonas = cmbZonas.EditValue.ToString().Replace(" ", "");
            string query = string.Format("EXEC [dbo].[VTA_SP_CUMPLIMIENTO_KILOS_LITROS_X_ZONA_GV] '{0}', '{1}', '{2}'", dtpFechaInicial.Value, dtpFechaFinal.Value, psZonas);
