...
namespace REH_Presentacion.Cobranza.Parametrizadores
{
    ...
    /// </summary>
    public partial class frmDiaNoLaborableZona : Form
    {
        #region Variables
        clsNComisiones loLogicaNegocio = new clsNComisiones();
        public List<DiaNoLaborableZona> ComisionDetalle = new List<DiaNoLaborableZona>();
        ...
        private void frmComisionDetalle_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                clsComun.gLLenarComboGrid(ref dgvDatos, loCombo, "CodeZona", false,true);
                
                clsComun.gDibujarBotonGrid(rpiBtnDel, dgvDatos.Columns["Del"], "Eliminar", Diccionario.ButtonGridImage.trash_16x16);

                dgvDatos.Columns["IdDiaNoLaborableZona"].Visible = false;
        ...
        private void btnAddManualmente_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (psCodeZona == Diccionario.Seleccione)
                {
                    foreach (var item in loCombo.Where(x => x.Codigo != Diccionario.Seleccione))
                    {
                        bsDatos.AddNew();
                        ((List<DiaNoLaborableZona>)bsDatos.DataSource).LastOrDefault().CodeZona = item.Codigo;
                        dgvDatos.Focus();
                ...
                {
                    bsDatos.AddNew();
                    ((List<DiaNoLaborableZona>)bsDatos.DataSource).LastOrDefault().CodeZona = psCodeZona;
                    dgvDatos.Focus();
                    dgvDatos.FocusedRowHandle = dgvDatos.RowCount - 1;
                    dgvDatos.FocusedColumn = dgvDatos.Columns[0];
                    dgvDatos.ShowEditor();
        ...
        private void btnAceptar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    ComisionDetalle = (List<DiaNoLaborableZona>)bsDatos.DataSource;
                    Close();
                }
                else
                {
        ...
        private string EsValido()
        {
            ...
            dgvDatos.PostEditor();
            var poLista = (List<DiaNoLaborableZona>)bsDatos.DataSource;
            var poListaRecorrida = new List<DiaNoLaborableZona>();
            int fila = 1;
