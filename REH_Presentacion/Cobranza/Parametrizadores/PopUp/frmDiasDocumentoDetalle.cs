...
namespace REH_Presentacion.Cobranza.Parametrizadores
{
    ...
    /// </summary>
    public partial class frmDiasDocumentoDetalle : Form
    {
        #region Variables
        clsNComisiones loLogicaNegocio = new clsNComisiones();
        public List<ZonaDiasDocumentoDetalle> Detalle = new List<ZonaDiasDocumentoDetalle>();
        ...
        /// </summary>
        public frmDiasDocumentoDetalle()
        {
            InitializeComponent();
        ...
        private void frmComisionDetalle_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                dgvDatos.Columns["IdZonaDiasDocumentoDetalle"].Visible = false;
                dgvDatos.Columns["IdZonaDiasDocumento"].Visible = false;

                dgvDatos.Columns["DiasDocumento"].Caption = "Días Documento";
                dgvDatos.Columns["DiasDocumentoAdd"].Caption = "Días Adicionales";
        ...
        private void btnAceptar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    Detalle = (List<ZonaDiasDocumentoDetalle>)bsDatos.DataSource;
                    Close();
                }
                else
                {
        ...
        private string EsValido()
        {
            ...
            dgvDatos.PostEditor();
            var poLista = (List<ZonaDiasDocumentoDetalle>)bsDatos.DataSource;
            var poListaRecorrida = new List<ZonaDiasDocumentoDetalle>();
            int fila = 1;
            foreach (var item in poLista)
            {
                if (poListaRecorrida.Where(x=>x.DiasDocumento == item.DiasDocumento).Count() > 0)
                {
                    psResult = string.Format("{0}Eliminar Fila # {1}, Ya existe una parametrización por el día: {2}\n", psResult, fila, item.DiasDocumento);
                }
