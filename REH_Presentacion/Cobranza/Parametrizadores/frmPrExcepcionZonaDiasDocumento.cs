...
namespace REH_Presentacion.Cobranza.Parametrizadores
{
    ...
    /// </summary>
    public partial class frmPrExcepcionZonaDiasDocumento : frmBaseTrxDev
    {
        #region Variables
        ...
        /// </summary>
        public frmPrExcepcionZonaDiasDocumento()
        {
            InitializeComponent();
        ...
        private void btnGrabar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    List<ZonaDiasDocumento> poLista = (List<ZonaDiasDocumento>)bsDatos.DataSource;
                    ...
                    {
                        string psMsg = loLogicaNegocio.gsGuardarZonaDiasDocumento(poLista, clsPrincipal.gsUsuario, clsPrincipal.gsTerminal);
        ...
        private void rpiBtnDel_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            try
            {
                ...
                var poLista = (List<ZonaDiasDocumento>)bsDatos.DataSource;
        ...
        private void rpiBtnDet_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            try
            {
                ...
                {
                    // Tomamos la entidad de la fila seleccionada
                    var poDetalle = poLista[piIndex].ZonaDiasDocumentoDetalle == null ? new List<ZonaDiasDocumentoDetalle>() : poLista[piIndex].ZonaDiasDocumentoDetalle.ToList();

                    frmDiasDocumentoDetalle frmComisionDetalle = new frmDiasDocumentoDetalle();
                    frmComisionDetalle.Detalle = poDetalle;
                    frmComisionDetalle.ShowDialog();

                    if (frmComisionDetalle.pbAcepto)
                    {
                        poLista[piIndex].ZonaDiasDocumentoDetalle = frmComisionDetalle.Detalle;
                    }
                }
            }
            catch (Exception ex)
        ...
        private void btnAddManualmente_Click(object sender, EventArgs e)
        {
            ...
            {
                bsDatos.AddNew();
                ((List<ZonaDiasDocumento>)bsDatos.DataSource).LastOrDefault().CodeZona = Diccionario.Seleccione;
                dgvDatos.Focus();
        ...
        private string lsEsValido()
        {
            ...
            var poListaRecorrida = new List<ZonaDiasDocumento>();
            int fila = 1;
            foreach (var item in poLista)
            {
                string psName = loComboZona.Where(x => x.Codigo.Contains(item.CodeZona)).Select(x => x.Descripcion).FirstOrDefault();
                if (item.CodeZona == Diccionario.Seleccione)
                {
                    psMsg = string.Format("{0}Seleccionar la zona en la Fila # {1}\n", psMsg, fila);
                }

                else if (poListaRecorrida.Where(x => x.CodeZona == item.CodeZona).Count() > 0)
                {
                    psMsg = string.Format("{0}Eliminar Fila # {1}, Ya existe una parametrización de la zona: {2}\n", psMsg, fila, psName);
                }
        ...
        private void lCargarEventosBotones()
        {
            ...
            bsDatos.DataSource = new List<ZonaDiasDocumento>();
            ...
            dgvDatos.Columns["ZonaDiasDocumentoDetalle"].Visible = false;
            dgvDatos.Columns["IdZonaDiasDocumento"].Visible = false;

            dgvDatos.Columns["CodeZona"].Caption = "Zona";
            dgvDatos.Columns["IdZonaDiasDocumento"].Caption = "Id";

            dgvDatos.Columns["IdZonaDiasDocumento"].OptionsColumn.AllowEdit = false;
            ...
            dgvDatos.Columns["CodeZona"].Width = 500;
        }

        private void lBuscar()
        { 
            bsDatos.DataSource = loLogicaNegocio.goConsultarExcepcionZonaDiasDocumento();
            gcDatos.DataSource = bsDatos;
