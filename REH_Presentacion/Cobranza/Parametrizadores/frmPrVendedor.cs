...
namespace REH_Presentacion.Cobranza.Parametrizadores
{
    ...
    /// </summary>
    public partial class frmPrVendedor : frmBaseTrxDev
    {
        #region Variables
        ...
        private List<Combo> loComboGrupoFactura = new List<Combo>();
        #endregion
        ...
        private void frmPrExcepcionClienteComision_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                loComboGrupoFactura = loLogicaNegocio.goConsultarComboGrupoFactura();
                lCargarEventosBotones();
                clsComun.gLLenarComboGrid(ref dgvDatos, loComboVendedor, "SlpCode", true);
                clsComun.gLLenarComboGrid(ref dgvDatos, loComboVendedor, "SlpCodeCurrent", false);
                clsComun.gLLenarComboGrid(ref dgvDatos, loComboGrupoFactura, "CodigoGrupoFactura", true);
        ...
        private void btnGrabar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    List<Vendedor> poLista = (List<Vendedor>)bsDatos.DataSource;
                    ...
                    {
                        string psMsg = loLogicaNegocio.gsGuardarVendedor(poLista, clsPrincipal.gsUsuario, clsPrincipal.gsTerminal);
        ...
        private void rpiBtnDel_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            try
            {
                ...
                var poLista = (List<Vendedor>)bsDatos.DataSource;
        ...
        private void btnAddManualmente_Click(object sender, EventArgs e)
        {
            ...
            {
                bsDatos.AddNew();
                ((List<Vendedor>)bsDatos.DataSource).LastOrDefault().SlpCode = 0;
                ((List<Vendedor>)bsDatos.DataSource).LastOrDefault().SlpCodeCurrent =0;
                ((List<Vendedor>)bsDatos.DataSource).LastOrDefault().CodigoGrupoFactura = Diccionario.Seleccione;
                dgvDatos.Focus();
        ...
        private string lsEsValido()
        {
            ...
            var poListaRecorrida = new List<Vendedor>();
            int fila = 1;
            foreach (var item in poLista)
            {
                string psVen1 = loComboVendedor.Where(x => x.Codigo.Contains(item.SlpCode.ToString())).Select(x => x.Descripcion).FirstOrDefault();
                string psVen2 = loComboVendedor.Where(x => x.Codigo.Contains(item.SlpCodeCurrent.ToString())).Select(x => x.Descripcion).FirstOrDefault();

                if (item.SlpCode.ToString() == Diccionario.Seleccione || item.SlpCodeCurrent.ToString() == Diccionario.Seleccione || item.CodigoGrupoFactura == Diccionario.Seleccione)
                {
                    psMsg = string.Format("{0}Verificar si están seleccionados el Vendedor, Vendedor Actual y el Código Vendedor en la Fila # {1}\n", psMsg, fila);
                }

                else if (poListaRecorrida.Where(x => x.SlpCode == item.SlpCode && x.SlpCodeCurrent == item.SlpCodeCurrent).Count() > 0)
                {
                    psMsg = string.Format("{0}Eliminar Fila # {1}, Ya existe una parametrización por: {2} y {3}\n", psMsg, fila, psVen1, psVen2);
                }
        ...
        private void lCargarEventosBotones()
        {
            ...
            bsDatos.DataSource = new List<Vendedor>();
            ...
            dgvDatos.Columns["SlpNameCurrent"].Visible = false;
            dgvDatos.Columns["Id"].Visible = false;

            dgvDatos.Columns["SlpCode"].Caption = "Vendedor";
            dgvDatos.Columns["SlpCodeCurrent"].Caption = "Vendedor Actual";
            ...
            dgvDatos.Columns["SlpCode"].Width = 250;
            dgvDatos.Columns["SlpCodeCurrent"].Width = 250;
            dgvDatos.Columns["CodigoGrupoFactura"].Width = 100;
        }

        private void lBuscar()
        { 
            bsDatos.DataSource = loLogicaNegocio.goConsultarVendedor();
            gcDatos.DataSource = bsDatos;
