...
namespace REH_Presentacion.Parametrizadores
{
    ...
    /// </summary>
    public partial class frmPaCuentaContableTipoTransporteAlmacen : frmBaseTrxDev
    {
        ...
        /// </summary>
        public frmPaCuentaContableTipoTransporteAlmacen()
        {
            InitializeComponent();
        ...
        private void frmPaCuentaContable_Load(object sender, System.EventArgs e)
        {
            try
            {
                ...
                clsComun.gLLenarCombo(ref cmbAlmacen , loLogicaNegocio.goSapConsultaComboCatalogoBodegas(), true);
                lbCargado = true;
                ...
                if (!string.IsNullOrEmpty(lsCodigoRubro))
                {
                    ...
                    cmbAlmacen.EditValue = lsCodigoRubro;
        ...
        private void btnGrabar_Click(object sender, EventArgs e)
        {
            try
            {
                if (lbEsValido())
                {
                    var psResult = loLogicaNegocio.gsGuardarCuentaContableTipoTransporteAlmacen(cmbTipoTransporte.EditValue.ToString(), cmbAlmacen.EditValue.ToString(), txtCuentaContable.Text.Trim(), clsPrincipal.gsUsuario, clsPrincipal.gsTerminal);
        ...
        private void gcParametrizaciones_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                if (dgvParametrizaciones.FocusedRowHandle >= 0)
                {
                    ...
                    cmbAlmacen.EditValue = dgvParametrizaciones.GetDataRow(dgvParametrizaciones.FocusedRowHandle).ItemArray[2].ToString();
                    lCargaCuentaContable();
                }
            }
            catch (Exception ex)
        ...
        private void lLimpiar()
        {
            ...
            if ((cmbAlmacen.Properties.DataSource as IList).Count > 0) cmbAlmacen.ItemIndex = 0;
        ...
        private bool lbEsValido()
        {
            ...
            if (cmbAlmacen.EditValue.ToString() == Diccionario.Seleccione)
            {
                XtraMessageBox.Show("Seleccione Almacén.", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return false;
            }
        ...
        private void lCargarParametrizaciones()
        {
            ...
            DataTable dt = loLogicaNegocio.gdtConsultaParametrizacionesTipoTranspoteAlmacen();
        ...
        {
            if (cmbAlmacen.EditValue.ToString() != Diccionario.Seleccione && cmbTipoTransporte.EditValue.ToString() != Diccionario.Seleccione)
            {
                txtCuentaContable.Text = loLogicaNegocio.gsConsultarCuentaContableTipoTransporteAlmacen(cmbAlmacen.EditValue.ToString(), cmbTipoTransporte.EditValue.ToString());
                lConsultarNombreCuenta();
            }
        }
