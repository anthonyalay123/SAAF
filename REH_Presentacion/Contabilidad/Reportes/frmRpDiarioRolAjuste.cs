...
namespace REH_Presentacion.Reportes
{
    ...
    /// </summary>
    public partial class frmRpDiarioRolAjuste : frmBaseTrxDev
    {
            ...
            try
            {
                ...
                if (lbEsValido())
                {
                    ...
                    if (poLista != null && poLista.Rows.Count > 0)
                    {
                        ...
                        clsComun.gSaveFile(gcDatos, "Diario_Ajuste" + ".xlsx", psFilter);     
                    }
                }
        ...
        private void lBuscar()
        {
            ...
            if (!string.IsNullOrEmpty(txtAnio.Text) && !string.IsNullOrEmpty(txtMes.Text))
            {

                if (!loLogicaNegocio.gbValidaPeriodoCerradoParaProvision(int.Parse(txtMes.Text), int.Parse(txtAnio.Text)))
                {
                    XtraMessageBox.Show("No es posible consultar diario el departamento de Talento Humano no ha Cerrado el Periodo a consultar", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }

                DataTable dt = loLogicaNegocio.goConsultaDataTable(string.Format("EXEC REHSPGCDIARIOPROVISIONAJUSTE '{0}','{1}'", txtAnio.Text, txtMes.Text));
            ...
            {
                XtraMessageBox.Show("Ingrese el año y mes para consultar.", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void frmRpDiarioRolAjuste_KeyDown(object sender, KeyEventArgs e)
        {
