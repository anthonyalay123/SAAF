    ...
    public partial class frmReportes : frmBaseTrxDev
    {
        ...
        private void btnImprimir_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                var pMenu = int.Parse(Tag.ToString().Split(',')[0]);
                if (pMenu == 315 || pMenu == 314)
                {
                    var psMsg = loLogicaNegocio.gsPresentaMensajePeriodosPendientes(int.Parse(txtPar1.Text), int.Parse(txtPar2.Text));
                ...
                {
                    System.Data.DataSet ds = new System.Data.DataSet();
                    var dt = loLogicaNegocio.goConsultaDataTable(string.Format("EXEC REHSPRPTSALDOVACACIONESCORTE '{0}','{1}' ", txtPar1.Text, txtPar2.Text));
                    dt.TableName = "HistoricoVac";
                    if (ds.Tables["HistoricoVac"] != null) ds.Tables.Remove("HistoricoVac");
                    ds.Merge(dt);
                    if (dt.Rows.Count > 0)
                    {
                        xrptHistoricoVacaciones xrptVinc = new xrptHistoricoVacaciones();
                        //Se establece el origen de datos del reporte (El dataset previamente leído)
                        xrptVinc.Parameters["parameter1"].Value = string.Format("PROVISIÓN DE VACACIONES {0}/{1}", clsComun.gsGetMes(int.Parse(txtPar2.Text)), txtPar1.Text);
                        xrptVinc.DataSource = ds;

                        //Se invoca la ventana que muestra el reporte.
                        xrptVinc.RequestParameters = false;
                        xrptVinc.Parameters["parameter1"].Visible = false;
        ...
        private void lBuscar()
        {
            ...
            {
                clsComun.gDibujarBotonGrid(rpiBtnShow, dgvDatos.Columns["107"], "107", Diccionario.ButtonGridImage.show_16x16);
