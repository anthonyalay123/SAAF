...
{
    public partial class frmRpProvisionesDecimoTercero : frmBaseTrxDev
    {
        clsNGestorConsulta loLogicaNegocio = new clsNGestorConsulta();
        BindingSource bsDatos = new BindingSource();

        public frmRpProvisionesDecimoTercero()
        {
            InitializeComponent();
        }
        ...
        private void btnImprimir_Click(object sender, EventArgs e)
        {
            try
            {
                if (lEsValido())
                {
                    ...
                    var poPeriodo = loLogicaNegocio.goConsultarPeriodoD3ro(psFechaFinal);
                    if (poPeriodo != null)
                    {
                        System.Data.DataSet ds = new System.Data.DataSet();
                        var dt = loLogicaNegocio.goConsultaDataTable(string.Format("EXEC REHSPHISTORICOPROVISIONESDT '{0}','{1}','{2}', '1'", poPeriodo.FechaInicio.ToString("dd/MM/yyyy"), psFechaFinal.ToString("dd/MM/yyyy"), cmbTipoReporte.EditValue.ToString()));
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            {
                                xrptProvisionesDecimoTercero xrpt = new xrptProvisionesDecimoTercero();

                                //Se establece el origen de datos del reporte (El dataset previamente leído)
                                xrpt.Parameters["parameter1"].Value = string.Format("PROVISIÓN DÉCIMO TERCER SUELDO\n{0} / {1}", clsComun.gsGetMes(psFechaFinal.Month), psFechaFinal.Year); ;
                            ...
                            {
                                xrptProvisionesDecimoTerceroCC xrpt = new xrptProvisionesDecimoTerceroCC();
        ...
        private void lBuscar()
        {
            ...
            if (lEsValido())
            {
                ...
                if (poPeriodo != null)
                {
                    var dt = loLogicaNegocio.goConsultaDataTable(string.Format("EXEC REHSPHISTORICOPROVISIONESDT '{0}','{1}','{2}'", poPeriodo.FechaInicio.ToString("dd/MM/yyyy"), psFechaFinal.ToString("dd/MM/yyyy"), cmbTipoReporte.EditValue.ToString()));
                    dgvDatos.Columns.Clear();
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        DateTime pdFechaCalculo = poPeriodo.FechaInicio;

                        int acum = 0;
                        gridBandIni.Columns.Add(dgvDatos.Columns[1]); // Tipo

                        if (cmbTipoReporte.EditValue.ToString() == "E")
                        {
                            dgvDatos.Columns[0].Visible = false;
                            gridBandIni.Columns.Add(dgvDatos.Columns[2]); // Empleado
                            gridBandIni.Columns.Add(dgvDatos.Columns[3]); // Centro Costo
                            gridBandIni.Columns.Add(dgvDatos.Columns[4]); // Saldo Anterior
                            acum = 5;
                        }
                        else
                        {
                            gridBandIni.Columns.Add(dgvDatos.Columns[1]); // Centro Costo
                            gridBandIni.Columns.Add(dgvDatos.Columns[2]); // Saldo Anterior
                            acum = 3;
                        }
