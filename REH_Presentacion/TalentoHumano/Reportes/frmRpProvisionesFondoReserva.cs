...
{
    public partial class frmRpProvisionesFondoReserva : frmBaseTrxDev
    {
        clsNGestorConsulta loLogicaNegocio = new clsNGestorConsulta();
        BindingSource bsDatos = new BindingSource();

        public frmRpProvisionesFondoReserva()
        {
            InitializeComponent();
        }
        ...
        private void btnImprimir_Click(object sender, EventArgs e)
        {
            try
            {
                if (lEsValido())
                {
                    ...
                    var dt = loLogicaNegocio.goConsultaDataTable(string.Format("EXEC REHSPHISTORICOPROVISIONESFR '{0}','{1}','{2}','1'", psFechaIni.ToString("dd/MM/yyyy"), psFechaFinal.ToString("dd/MM/yyyy"), cmbTipoReporte.EditValue.ToString()));
        ...
        private void lBuscar()
        {
            if (lEsValido())
            {
                ...
                DateTime psFechaIni = new DateTime(int.Parse(txtAnio.Text), 1, 1); //dtpFechaInicial.DateTime.Date.ToString("dd/MM/yyyy");
                DateTime psFechaFinal = new DateTime(int.Parse(txtAnio.Text), int.Parse(txtMes.Text), 1).AddMonths(1).AddDays(-1); //dtpFechaFinal.DateTime.Date.ToString("dd/MM/yyyy");
                var dt = loLogicaNegocio.goConsultaDataTable(string.Format("EXEC REHSPHISTORICOPROVISIONESFR '{0}','{1}','{2}','0'", psFechaIni.ToString("dd/MM/yyyy"), psFechaFinal.ToString("dd/MM/yyyy"), cmbTipoReporte.EditValue.ToString()));
