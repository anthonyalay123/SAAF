...
{
    public partial class frmRpProvisionesVacacionesCentroCosto : frmBaseTrxDev
    {
        clsNGestorConsulta loLogicaNegocio = new clsNGestorConsulta();
        BindingSource bsDatos = new BindingSource();

        public frmRpProvisionesVacacionesCentroCosto()
        {
            InitializeComponent();
        }
        ...
        private void btnImprimir_Click(object sender, EventArgs e)
        {
            try
            {
                if (lEsValido())
                {
                    ...
                    var dt = loLogicaNegocio.goConsultaDataTable(string.Format("EXEC REHSPHISTORICOPROVISIONESRESUMEN '{0}','{1}'", psFechaIni, psFechaFinal));
                    dt.TableName = "ProvisionVacacionesCC";
                    ds.Merge(dt);

                    if (dt.Rows.Count > 0)
                    {
                        xrptProvisionesVacacionesCentroCostoResumen xrpt = new xrptProvisionesVacacionesCentroCostoResumen();
