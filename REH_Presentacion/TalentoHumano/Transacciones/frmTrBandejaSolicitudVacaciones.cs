...
{
    public partial class frmTrBandejaSolicitudVacaciones : frmBaseTrxDev
    {
        ...
        public frmTrBandejaSolicitudVacaciones()
        {
        ...
        private void btnAprobar_Click(object sender, EventArgs e)
        {
            ...
            {
                dgvDatos.PostEditor();
                var poLista = ((List<BandejaSolicitudVacaciones>)bsDatos.DataSource).Where(x => x.Sel).ToList();
                ...
                if (poLista.Count() > 0)
                {
                    ...
                    if (dialogResult == DialogResult.Yes)
                    {
                        ...
                        {
                            string Aprobar = loLogicaNegocio.gsVerAprobaciones(Diccionario.Tablas.Transaccion.SolicitudVacaciones, item.Id, clsPrincipal.gsUsuario);
                            if (Aprobar == "")
                            {
                ...
                if (poLista.Count() > 0)
                {
                    ...
                    if (dialogResult == DialogResult.Yes)
                    {
                        ...
                        foreach (var item in poLista)
                        {
                            ...
                            {
                                psMsg = psMsg + "Solicitud No. " + item.Id + " debe tener comentario para envíar a corregir. \n";
                            }
                        }
                        ...
                        if (string.IsNullOrEmpty(psMsg))
                        {
                            ...
                            foreach (var item in poLista)
                            {
                                ...
                                if (psMess != "")
                                {
                                    IdNoActualizadas += "Solicitud No. " + item.Id + ": " + psMess + "\n";
                                }
                            }
                            lListar();
                ...
                if (poLista.Count() > 0)
                {
                    ...
                    if (dialogResult == DialogResult.Yes)
                    {
                        ...
                        foreach (var item in poLista)
                        {
                            if (string.IsNullOrEmpty(item.Comentario))
                            {
                                psMsg = psMsg + "Solicitud No. " + item.Id + " debe tener comentario para envíar a Rechazar. \n";
                            }
                        }
        ...
        private void lListar()
        {
            ...
            var poObject = loLogicaNegocio.goListarBandejaSolicitudVacaciones(clsPrincipal.gsUsuario, Int32.Parse(menu[0]));
