...
Public Class Form1
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles btnConsulta.Click
        ...
        Try
            ...
            If chbComedor.Checked = False Then
                ...
                If chkTodos.Checked Then
                    sql = "SELECT 
                        ...
                        CASE WHEN CONVERT(VARCHAR,t1.entrada,8) = CONVERT(VARCHAR,t1.horario,8) THEN '' ELSE ISNULL(CONVERT(VARCHAR,t1.horario,8),'') END horario,
                        --CASE WHEN t1.entrada = t1.horario THEN '' ELSE t1.horario END horario, 
                        --t1.horario horario,
                        --CAST(t2.fecha AS DATE) fecha,
                        CONVERT(VARCHAR,t2.fecha,101) fecha, 
                        --t1.entrada,
                        CASE WHEN t1.entrada BETWEEN '00:00:00' AND '15:59:59' THEN t1.entrada ELSE '' END entrada,
                        t1.transformado,
                        t1.calculo,
                        --t1.tiempoRetraso,
                        --t1.atrasado,
                        CASE WHEN t1.entrada BETWEEN '00:00:00' AND '15:59:59' THEN t1.tiempoRetraso ELSE '00:00:00' END tiempoRetraso,
    	                CASE WHEN t1.entrada BETWEEN '00:00:00' AND '15:59:59' THEN t1.atrasado ELSE 'NO' END atrasado,
            ...
            Else
                ...
                sql2 = "SELECT D1.codigo,D1.empleado,D1.departamento, CONVERT(char(8), DATEADD(ss, SUM(D1.soloRetraso), '19000101'), 108) retrasoTotal FROM (
                        ...
                        LEFT JOIN hr_department dep ON dep.id=emp.department_id
  		                LEFT JOIN (SELECT id,CONVERT(VARCHAR,timetable_start,8) empiezaDia,timetable_latecome periodoGracia,CONVERT(VARCHAR,DATEADD(minute,timetable_latecome,timetable_start),8) entradaGeneral FROM att_timetable) general on general.id=1
                        LEFT JOIN vw_horariosxempleado2 hora on hora.employee_id=emp.id
                        WHERE emp.emp_active = '1'
                        ) AS D1 WHERE  d1.fecha BETWEEN ('" & fechaDesde & "') and ('" & fechaHasta & "') 
                        "
                ...
                If chkTodos.Checked Then
                    sql = "SELECT 
                        ...
                        --t1.tiempoRetraso,
                        --t1.atrasado,
                        CASE WHEN DATEDIFF(ss,CAST(t1.entrada AS datetime),CONVERT(varchar,t1.horario,8)) > 1800 THEN CONVERT(varchar,DATEADD(second, DATEDIFF(ss,CAST(DATEADD(SS,1800,t1.entrada) AS datetime),CONVERT(varchar,t1.horario,8)),CAST('00:00:00' AS TIME)),8) ELSE '00:00:00' END tiempoRetraso,
                        CASE WHEN DATEDIFF(ss,CAST(t1.entrada AS datetime),CONVERT(varchar,t1.horario,8)) > 1800 THEN 'SI' ELSE 'NO' END atrasado,
                        '' falta,
                        '' estado
                        FROM relacionEmpFecha t2
                        LEFT JOIN
